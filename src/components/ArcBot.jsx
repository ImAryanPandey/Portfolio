...
import { X, Send, ChevronRight, Cpu } from 'lucide-react';

const ArcBot = ({ isOpen, onClose }) => {
  ...
  const handleCommand = async (e) => {
    ...
    try {
      ...
      setHistory(prev => [...prev, { type: 'system', content: `ERROR: ${err.message || 'CONNECTION SEVERED'}` }]);
    } finally {
      setLoading(false);
    }
  };
            ...
            {loading && (
              <div className="flex justify-start">
                <div className="flex items-center gap-1 p-3 bg-[#111] border border-white/10 rounded-md">
                  <span className="w-1.5 h-1.5 bg-blue-500/50 rounded-full animate-bounce" style={{ animationDelay: '0ms' }}></span>
                  <span className="w-1.5 h-1.5 bg-blue-500/50 rounded-full animate-bounce" style={{ animationDelay: '150ms' }}></span>
                  <span className="w-1.5 h-1.5 bg-blue-500/50 rounded-full animate-bounce" style={{ animationDelay: '300ms' }}></span>
                </div>
              </div>
            )}
